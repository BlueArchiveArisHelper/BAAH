...
if __name__ == '__main__':
    ...
    if sys.version_info[:2] == (3, 12):
        for i,core_list_item in enumerate(core_list):
            if "onnxruntime" in core_list_item:
                core_list[i] = "onnxruntime==1.17.0\n"
                break
    ...
    if "--core" in sys.argv:
        print("生成核心依赖列表 requireforyou.txt for core dependencies.")
        with open(os.path.join(SCRIPT_DIR, 'requireforyou.txt'), 'w') as f:
            f.writelines(core_list)
    elif "--build" in sys.argv:
        print("生成构建依赖列表 requireforyou.txt for build dependencies.")
        with open(os.path.join(SCRIPT_DIR, 'requireforyou.txt'), 'w') as f:
